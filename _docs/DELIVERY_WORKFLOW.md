...
# 2. Filter: keep backend/ and frontend/ (same subfolder structure as NL)
git filter-repo \
  --path backend/ \
  --path frontend/ \
  --force
...
After filter-repo, both delivery repos have this structure (same as NL):

```
se-origin / sd-origin root
...
git fetch upstream
git reset --hard upstream/develop
git filter-repo --path backend/ --path frontend/ --force
git push se-origin develop:be-delivery --force-with-lease
...
# SE delivery (after squash merge to develop in NL)
cd nutri-ledger-se-delivery
git fetch upstream && git reset --hard upstream/develop
git filter-repo --path backend/ --path frontend/ --force
git push se-origin develop:be-delivery --force-with-lease
