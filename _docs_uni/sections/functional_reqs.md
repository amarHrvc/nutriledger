...
- The token carries an expiry configured via `sanctum.expiration`.
...
- Only the token used for the logout request is deleted; other active tokens for the same user are unaffected.
...
- A deactivated account returns HTTP 401 with `{ "message": "Account is deactivated" }`.
- No information is leaked about whether the email exists.

---

As the system, I temporarily block login attempts from a requester after a configurable number of consecutive failures so that brute-force attacks are mitigated.
...
> **Implementation note:** Configured via `RateLimiter::for('login', fn ($r) => Limit::perMinute(5)->by($r->ip()))` in `AppServiceProvider` and applied as `throttle:login` on `POST /api/login`.
...
> ⚠️ **GAP:** `UpdateUserRequest::authorize()` only passes for admins. No dedicated "update own profile" endpoint exists. Non-admin users cannot currently update their own profile.
...
> ⚠️ **GAP:** `UpdateUserRequest` accepts a `role` field with no restriction against self-update. An admin can change their own role via `PATCH /api/users/{own-admin-id}`.
...
- The deactivated user's record remains visible to admins via `GET /api/users` and `GET /api/users/{id}`.
...
> ⚠️ **GAP:** `UserController::destroy()` calls only `$user->delete()`. It does not call `$user->tokens()->delete()`. Active tokens remain valid after deactivation.
...
> **Implementation note:** Wrapped in `DB::transaction()` via `PatientService::createPatient()`.
...
> ⚠️ **GAP:** `PatientController::index()` does not apply any search/filter logic. The `search` query parameter is not handled.
...
> ⚠️ **GAP (partial):** Restoring a user via `POST /api/users/{id}/restore` cascades to restore the patient (via `User::restored` model event → `Patient::withTrashed()->restore()` → `Patient::restored` → `PatientSocioeconomic::restore()`). This covers the case where the patient was deactivated as a side-effect of user deactivation. However, when a patient is deactivated independently via `DELETE /api/patients/{id}` (without deactivating the user), there is no restore path — no `POST /api/patients/{id}/restore` endpoint exists.
...
> **Implementation note:** `doctor_id` is hardcoded to `auth()->id()` in `VisitController::store()` and is not present in `StoreVisitRequest` validation rules.
...
- Each visit includes the associated doctor data (eager-loaded).
- Response includes `data`, `meta`, and `links` pagination keys.
...
> **Implementation note:** Authorization enforced via `UpdateVisitRequest::authorize()` → `VisitPolicy::update()`.
