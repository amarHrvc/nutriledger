...
- [ ] T3 [P] Create BFF route `frontend/src/app/api/patients/[id]/vitals/route.ts` — `GET` handler using `patientsVitalsHistory` (confirmed generated name from Foundation T8); forward `from`, `to`, `page`, and `per_page` query params from `req.url` search params
...
- Actual Orval function names (confirmed in Foundation T8) must be used verbatim in VSR1 T6 and VSR2 T3 — confirmed names: `patientsVisitsVitalsShow`, `patientsVisitsVitalsStore`, `patientsVisitsVitalsUpdate`, `patientsVisitsVitalsDestroy`, `patientsVitalsHistory` (history was `patientsVitals` in the original spec — use `patientsVitalsHistory`)
